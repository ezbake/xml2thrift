...
class SchemaAttributeType implements SchemaType {
	...
	public boolean process(Attributes attributes, SchemaMapper mapper) {
		...
		mapper.setType(attributes.getValue("type"));
		
		if (mapper.getType() != null) {
			mapper.getMap().addValues(mapper.getName(), mapper.getType());
		} else {
			mapper.getMap().addValues(mapper.getName(), "xs:string"); // default to string type ... for now
		}
		
		if (mapper.getRoot().isEmpty()) {
			mapper.setRoot(mapper.getName());
		}
		return true;
	}
